...
use rand::Rng;
use serenity::{
    model::{application::{interaction::{Interaction, InteractionResponseType}, component::ButtonStyle,},
...
pub async fn dead_chat(ctx: &serenity::Context, channel_id: ChannelId) -> Result<(), serenity::Error> {
    let dead_gifs = ["https://media.tenor.com/PFyQ24Kux9UAAAAC/googas-wet.gif", 
    ...
    channel_id.say(&ctx.http, dead_gifs[random_number(dead_gifs.len())]).await?;
    Ok(())
}

pub async fn embed_builder<S: ToString>(ctx: &serenity::Context, message: &serenity::Message, title: S, url: S, colour: Colour) {
    let _ = message
        .channel_id
...
pub fn emoji_react(emoji: &str) -> ReactionType {
    ...
    let id;
    match emoji {
        "fabseman_willbeatu" => {id = 1135252520785150012;}
        _ => {id = 1135252520785150012;}
    }
...
pub async fn spoiler_message(ctx: &serenity::Context, message: &serenity::Message, text: String) {
    ...
    for attachment in &message.attachments {
        ...
        if index == 0 {
            webhook_file(&ctx, message, username, &avatar_url, &text, filename.to_string(), 0).await;
            index = 1;
        }
        else {
            webhook_file(&ctx, message, username, &avatar_url, &text, filename.to_string(), 1).await;
        }
...
pub async fn webhook_message<S: ToString>(ctx: &serenity::Context, message: &serenity::Message, name: S, url: &str, output: S) {
    ...
    if let Some(existing_webhook) = existing_webhooks.iter().find(|webhook| webhook.name.as_deref() == Some("fabsemanbots")) {
        let _ = existing_webhook.execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).content(output)}).await;
    } 
    else {
        let new_webhook = message.channel_id.create_webhook_with_avatar(ctx, "fabsemanbots", url).await;
        let _ = new_webhook.expect("rip webhooks").execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).content(output)}).await;
    }
}

pub async fn webhook_file<S: ToString>(ctx: &serenity::Context, message: &serenity::Message, name: S, url: &str, text: S, path: String, mode: i32) {
    let channel_id = message.channel_id;
    let existing_webhooks = match channel_id.webhooks(&ctx.http).await {
        Ok(webhooks) => webhooks,
        Err(err) => {
    ...
    if mode == 0 {
        if let Some(existing_webhook) = existing_webhooks.iter().find(|webhook| webhook.name.as_deref() == Some("fabsemanbots")) {
            let _ = existing_webhook.execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).content(text).add_file(AttachmentType::Path(path.as_ref()))}).await;
        } 
        else {
            let new_webhook = message.channel_id.create_webhook_with_avatar(ctx, "fabsemanbots", url).await;
            let _ = new_webhook.expect("rip webhooks").execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).content(text).add_file(AttachmentType::Path(path.as_ref()))}).await;
        }
    }
    else {
        if let Some(existing_webhook) = existing_webhooks.iter().find(|webhook| webhook.name.as_deref() == Some("fabsemanbots")) {
            let _ = existing_webhook.execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).add_file(AttachmentType::Path(path.as_ref()))}).await;
        } 
        else {
            let new_webhook = message.channel_id.create_webhook_with_avatar(ctx, "fabsemanbots", url).await;
            let _ = new_webhook.expect("rip webhooks").execute(&ctx.http, false, |w| {w.username(name).avatar_url(url).add_file(AttachmentType::Path(path.as_ref()))}).await;
        }
    }
}
