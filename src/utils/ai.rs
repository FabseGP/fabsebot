	...
	if message.content.eq_ignore_ascii_case("clear") {
		...
		if let Some(utils_config) = UTILS_CONFIG.get() {
				...
				get_model_config(&utils_config.fabseserver.text_gen_model);
			{
				...
				if static_set {
					if !known_user
						&& let Ok(author_member) =
							guild_id.member(&ctx.http, message.author.id).await
						&& let Some(author_roles) = author_member.roles(&ctx.cache)
					{
						...
						conversations.lock().await.static_info.users.insert(
							author_id_u64,
							format!(
						...
						if let Some(guild) = message.guild(&ctx.cache) {
							convo_lock.static_info.guild_desc = format!(
								"\nThe guild you're currently talking in is named {} with this \
								 description {}, have {} members and have {} channels with these \
								 names {}, current channel name is {}. {}",
								guild.name,
								guild
									.description
									.as_ref()
			...
			if let Some(reply) = &message.referenced_message {
				...
				write!(
					system_content,
					"\n{author_name} replied to a message sent by: {ref_name} and had this \
					 content: {}",
					reply.content
				)?;
			}
						...
						.get(&target.id.get())
					{
						...
						let target_desc = format!(
							"\n{target_name} was mentioned (global name is {target_global_name}). \
							 Roles: {target_roles}. Profile picture: {pfp_desc}. Joined this \
							 guild at this date: {target_joined_guild}"
						);
			...
			if !message.attachments.is_empty() {
				...
				for attachment in &message.attachments {
					if let Some(content_type) = attachment.content_type.as_deref()
						...
						&& let Some(desc) =
							ai_image_desc(&attachment.download().await?, Some(&message.content))
								.await
					{
					...
					.await && let Ok(resp_text) = resp.text().await
			{
				...
				Selector::parse("article.result-default p.content").map_or(
					None,
					|snippet_selector| {
						Some(
							parsed_page
			...
			warn!("Failed to get UTILS_CONFIG");
		}
	}
	Ok(())
}
