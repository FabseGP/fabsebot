...
- More user commands!
- DAVE-support
- Consolidate cache fields
- Track errors + move to mimalloc v3
- WIP better error handling
- Migrate from db <-> cache layer
- HUUUUGE refactoring!
- Forgejo action
- Schedule renovate
...
- Avoid cloning user settings on startup
- Avoid storing duplicate ids
- Prefix row may not exist
- Track playback errors
- Query only related rows for word tracking/reaction
- Move more queries to query_scalar
- Properly enable zstd transport compression
- Use slices
- Catch missing guild or user settings
- Reply to message when sending container-components
- Clearer error when joining empty voice channel
- Don't need to acquire that many db conns
- Cow -> String
- Scope reference
- Llama.cpp only supports jpeg :/
- Versioning + added renovate
- Don't rebase prs
- Empty github token (limited to 60 requests per hour)
- Override github token
- Add github secret
- Force renovate to use nightly rust
- Use arg
- Use semver
- Ignore rust tool
- Add cargo rule
...
- Remove unused env
- Set IGNORED_TOOLS
- Wider range for renovate schedule
- Once every saturday
...
- Update deps
- Log_error-function
- Update deps + push use of transactions
- Init cargo deny
- Formatting
- Update deps
- Update deps
